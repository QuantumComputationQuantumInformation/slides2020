...
From
\begin{align*}
\emm{R_z(\theta)(iX) R_z(-\theta) (-iX)} = R_z(2\theta)
\end{align*}
any $Z$-rotation has the group commutator decomposition.
For some unitary $S$, $U = SR_z(\eta)S^\dagger$ for some $\eta\in\mathbb{R}$.
Hence, $U$ has a group commutator decomposition $U = VWV^\dagger W^\dagger$ 
for $V=S\emm{R_z(\eta/2)}S^\dagger$ and $W:=S\emm{(iX)}S^\dagger$.
\end{proof}
...
\small
\begin{theorem}
For any $U\in\mathsf{SU}(2)$, there exist $V,\,W\in\mathsf{SU}(2)$ such that $U = VWV^\dagger W^\dagger$ for some $V,\,W$ satisfying $\|I-V\|<c_{\mathsf{GC}}\sqrt{\|I-U\|}$ and $\|I-W\|<c_{\mathsf{GC}}\sqrt{\|I-U\|}$
...
\begin{enumerate}
\setlength{\itemsep}{2em}
\item Show a quantum circuit for a controlled-Hadamard gate using arbitrary single-qubit gates and CNOT gates.
